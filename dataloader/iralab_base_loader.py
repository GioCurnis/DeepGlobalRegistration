...
class CollationFunctionFactoryForIRALABBenchmark:
  def __init__(self, concat_correspondences=True, collation_type='default'):
    ...
    logging.info(f'finisco init collationfunction')
  ...
  def collate_pair_fn(self, list_data):
    ...
    xyz0, xyz1, coords0, coords1, feats0, feats1, matching_inds, tf, itf, extra_packages = list(
        zip(*list_data))
    ...
    trans_batch = torch.from_numpy(np.stack(itf)).float()
    ...
    logging.info(f'finisco collate pair')
    return {
        'pcd0': xyz0,
        'pcd1': xyz1,
...
class IRALABBenchmarkPairDataset(torch.utils.data.Dataset):
  ...
  def __init__(self,
               ...
               transform=None,
               config=None):
    self.dataframe = pd.read_csv(test_file, sep=" ", header=0)
    self.run = run
    self.files = []
    self.data_objects = []
    ...
    for index, row in self.dataframe.iterrows():
      self.files.append((row['id'], row['source'], row['target']))
